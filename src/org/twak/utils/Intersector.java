...
public class Intersector 
{
    ...
    public List<Collision> intersectLines (Iterable<Line> a)
    {
        ...
        // get all events at the equiv location
        while (events.size() > 0)
        {
...
//                    return Double.compare( o1.start.x, o2.start.x );\
            ...
            if ( horiz != null )
            {
                ...
                if ( notHoriz.isVert() ) // avoid rounding errors
                    x = notHoriz.start.x;

                if (processing.equals( notHoriz.start))
                    x = notHoriz.start.x;

                if (processing.equals( notHoriz.end))
                    x = notHoriz.end.x;

//                if (processing.equals( horiz.start))
//                    x = horiz.start.x;
//
//                if (processing.equals( horiz.end))
//                    x = horiz.end.x;
